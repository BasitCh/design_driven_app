...
class Gallery3D extends StatefulWidget {
  ...
  final List<MyWalletCardItem> widgets;
  ...
  const Gallery3D({
    ...
    this.height = 350,
    this.width = 280,
    this.itemConfig = const GalleryItemConfig(),
...
class _Gallery3DState extends State<Gallery3D>
  ...
  final double _minScale = 0.8; //最小缩放值
  ...
  void initState() {
    if (widget.itemCount < 3) {
      _unitAngle = 270 / widget.itemCount;
    } else {
  ...
  void _initGalleryTransformInfoMap() {
    ...
    for (var i = 0; i < widget.itemCount; i++) {
      int itemAngle;
      if (widget.itemCount < 3) {
        itemAngle = getFinalAngle(405 + _unitAngle * i);
      } else {
        itemAngle = getFinalAngle(180 + _unitAngle * i);
      }
